...
- **Chat needs the user's OpenAI API key, encryption key, and a database.** With auth alone the dashboard shows a setup gate until the signed-in user saves an OpenAI key wrapped with their own encryption passphrase (never stored). After the unlock session expires they unlock with the encryption key again. Chat also returns 503 without `DATABASE_URL` (quotas and key storage fail closed). Set `DATABASE_URL` in `.env.local`, then paste keys in the UI. `BLOB_READ_WRITE_TOKEN` (Vercel Blob) is only needed for receipt image/CSV uploads. Optional `CREDENTIALS_ENCRYPTION_KEY` (64 hex chars) protects the unlock-session cookie; otherwise a hash of `CLERK_SECRET_KEY` is used.
