...
- **Chat needs an external key and a database.** With auth alone the dashboard loads, but sending a message returns 500 without `AI_GATEWAY_API_KEY`, or 503 without `DATABASE_URL` (quotas fail closed — AI is never served unmetered). Set both in `.env.local` to exercise the assistant. Without `DATABASE_URL` the dashboard shows a banner that history is not saved and chat is disabled. `BLOB_READ_WRITE_TOKEN` (Vercel Blob) is only needed for receipt image/CSV uploads.
